 ...
 */
export function parseLch(input: string): LCHColor | null {
  const match = /^lch\(\s*(.*?)\s*\)$/i.exec(input);
  if (!match) return null;

  const body = match[1];
  if (!body) return null;

  // Reject comma syntax — lch() has no legacy comma form
...
/**
 * Build an LCHColor from parsed channels and an optional alpha token.
...
function parseSpaceSyntax(body: string): LCHColor | null {
